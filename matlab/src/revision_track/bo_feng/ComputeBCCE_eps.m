...
if size(distribution_parameters,1) == 0
    Psi = ones(s2,1);
    % Initialize marg_distrib for uniform case
    % Need to match the NGrid_lambda dimension
    marg_distrib = ones(s, NGrid_lambda) / s;
elseif size(distribution_parameters,1) == 3
    % Old format: distribution_parameters contains {'Normal', mu, sigma}
    % Initialize marg_distrib for the old approach
    ...
    for nd = 1:NGrid_lambda
        if strcmp(distribution_parameters{1,nd},'Normal')
            ...
            % Extract marginal distribution for epsilon calculations
            % For Normal distribution, we need to compute the marginal at the type space points
            marg_distrib(:,nd) = pdf('normal', type_space{1,1}, distribution_parameters{2,nd}(1), distribution_parameters{3,nd}(1,1));
            ...
            % Extract marginal distribution for epsilon calculations
            marg_distrib(:,nd) = pdf(distribution_parameters{1,nd}, type_space{1,1}, distribution_parameters{2,nd}(1), distribution_parameters{3,nd}(1,1));
            marg_distrib(:,nd) = marg_distrib(:,nd) ./ sum(marg_distrib(:,nd), 1);
        end 
    end
elseif size(distribution_parameters,1) == 1
    % New format: distribution_parameters contains probability mass vectors directly
    % Initialize marg_distrib for the new approach
    marg_distrib = zeros(s, NGrid_lambda);
    
    for nd = 1:NGrid_lambda
        % Each distribution_parameters{nd} is a probability vector
        prob_vector = distribution_parameters{nd};
        
        % Check if this is a probability vector (should sum to 1 and have s elements)
        if length(prob_vector) == s && abs(sum(prob_vector) - 1) < 1e-10
            % Store the marginal distribution for epsilon calculations
            % Ensure it's a column vector for consistency
            marg_distrib(:,nd) = prob_vector(:);
            
            % We need to create a joint distribution from the marginal distribution
            % The current system expects s² elements for NAg players with s types each
            % We'll create the joint distribution assuming independence between players
            % This replicates the logic from the main file where Psi(kk) = marg_distrib(ii)*marg_distrib(jj)
            
            joint_prob = zeros(s2, 1);
            kk = 0;
            for ii = 1:s
                for jj = 1:s
                    kk = (ii-1)*s + jj;
                    joint_prob(kk) = prob_vector(ii) * prob_vector(jj);
                end
            end
            Psi(:,nd) = joint_prob;
        else
            % Fallback: treat as uniform distribution
            marg_distrib(:,nd) = ones(s,1) / s;
            Psi(:,nd) = ones(s2,1) / s2;
        end
    end
end
...
% This section is no longer used with the new probability mass approach
% Commented out for compatibility with new format
...
for np = 1:NAlpha
    ...
    %eps_fin = repmat(marg_distrib(:,nd)',1,NAg*a).*repmat(sqrt(marg_distrib(:,nd))',1,NAg*a).*repmat(eps,1,NAg*a);
     eps_fin = repmat(sqrt(marg_distrib(:,nd))',1,NAg*a).*repmat(eps,1,NAg*a);
    else
    %eps_fin = repmat(marg_distrib(:,nd)',1,NAg*a).*repmat(eps,1,NAg*a);
    eps_fin = repmat(eps,1,NAg*a);
    end
