...
% Hold the OBEDIENCE budget (and thus epsilon) FIXED at alpha_R=0.045 across all
% slacked runs, and use the SAME-epsilon exact baseline (alpha_set=0.045), so only
% the CONSISTENCY radius varies. Tolerance is set above SeDuMi's per-solve noise
% (~2e-4 here, established by the box regression check).
aR = 0.045; TOL = 2e-3;
base = struct('maxiters_values', N, 'alpha_set', aR, 'switch_eps', 10, ...
...
% exact baseline at epsilon(alpha_R)
rng(12345); VV_ex = solve(base); VV_ex = VV_ex(1,:);
% L1 at r=0 (alpha_C = 2^M_C - 2 makes the BHC radius exactly 0), same epsilon
o = base; o.consistency_slack_kind = 'L1'; o.alpha_R = aR; o.alpha_C = 2^M_C - 2;
rng(12345); VV_l1z = solve(o); VV_l1z = VV_l1z(1,:);
% L1 at alpha_C = 0.005 (r>0), same epsilon
o = base; o.consistency_slack_kind = 'L1'; o.alpha_R = aR; o.alpha_C = 0.005;
rng(12345); VV_l1 = solve(o); VV_l1 = VV_l1(1,:);
% L1 at alpha_C = 0.0005 (larger r), SAME epsilon (alpha_R fixed) -> clean r-monotone test
o = base; o.consistency_slack_kind = 'L1'; o.alpha_R = aR; o.alpha_C = 0.0005;
rng(12345); VV_l1b = solve(o); VV_l1b = VV_l1b(1,:);
% box at alpha_C = 0.005, same epsilon (regression / noise calibration)
o = base; o.consistency_slack_kind = 'box'; o.alpha_R = aR; o.alpha_C = 0.005;
rng(12345); VV_bx = solve(o); VV_bx = VV_bx(1,:);

fprintf('radii @N=40k: r_L1(.005)=%.4f r_L1(.0005)=%.4f r_box(.005)=%.4f\n', ...
    df.solvers.compute_consistency_slack(M_C,0.005,N,'L1'), ...
    df.solvers.compute_consistency_slack(M_C,0.0005,N,'L1'), ...
    df.solvers.compute_consistency_slack(M_C,0.005,N,'box'));

% (1) r=0 == exact (same epsilon)
d0 = max(abs(VV_l1z - VV_ex));
fprintf('(1) L1 r=0 reproduces exact: max|dVV|=%.3e -> %s\n', d0, tf(d0 < 1e-9));
ok = ok && d0 < 1e-9;
% (2) L1 relaxes exact (consistency only; epsilon fixed)
v2 = max(VV_l1 - VV_ex);
fprintf('(2) L1(r>0) <= exact: max(VV_L1-VV_exact)=%.3e -> %s\n', v2, tf(v2 < TOL));
ok = ok && v2 < TOL;
% (3) monotone in r, epsilon fixed
v3 = max(VV_l1b - VV_l1);
fprintf('(3) larger r relaxes more (alpha_C .0005<=.005): max(dVV)=%.3e -> %s\n', v3, tf(v3 < TOL));
ok = ok && v3 < TOL;
% (4) box relaxes exact (regression)
v4 = max(VV_bx - VV_ex);
fprintf('(4) box(r>0) <= exact: max(VV_box-VV_exact)=%.3e -> %s\n', v4, tf(v4 < TOL));
ok = ok && v4 < TOL;
% counts
ne = nnz(VV_ex<=IDTOL); nl = nnz(VV_l1<=IDTOL); nb = nnz(VV_l1b<=IDTOL); nx = nnz(VV_bx<=IDTOL);
fprintf('identified counts: exact=%d  L1(.005)=%d  L1(.0005)=%d  box(.005)=%d (expect exact<=L1<=L1b)\n', ne, nl, nb, nx);
ok = ok && (ne <= nl) && (nl <= nb);
